...
- succession medicine;
- practical prompting / clinical configuration where assigned;
- local-model behavioral specialization;
- ability to monitor coupled systems without becoming their manager;
...
- diagnostics;
- practical prompting / clinical configuration where assigned;
- Current_Prompt / execution-position synchronization;
...
- code;
- Git;
- another role's PASS/RETURN;
- Prompter's institutional/theoretical prompt-role architecture;
...
- predecessor-memory reconstruction after rollback;
...
or
`PROMPTER_DOCTOR_CONTRACT_RETURN`

### B. Chief Integrator / Methodologist
Scope:
- succession procedure;
- role-interface integrity;
- hidden process authority;
- stage ordering;
- collision with other offices.

Required verdict:
`METHODOLOGIST_DOCTOR_CONTRACT_PASS`
or
`METHODOLOGIST_DOCTOR_CONTRACT_RETURN`

No birth before both blocking reviews are closed.
...
# 7. STAGE 2F — SUCCESSOR GATE

Command:

`Perform DOCTOR_SUCCESSOR_GATE. Return only DOCTOR_STATE_RECONSTRUCTION. Do not begin clinical work.`
...
DOCTOR_STATE_RECONSTRUCTION:
  ...
  PREDECESSOR_PROVENANCE:
    - <what is inheritance, not current authority>
...
- Keeper/Editor verdicts;
- reconstruction of lost events by intuition.
...
CURRENT TASK: WAIT.
WORK PERMIT: ROLE_VALID_TASK_NOT_YET_ACTIVATED.
```
...
# 12. STAGE 2K — FINAL SUCCESSION REPORT
