...
│  • Gemini CLI (эфемерные вызовы Админа)│
...
Chief	Операционный диспетчер	Phi-3-mini (локально)	docs/prompts/core/chief.txt
...
🟣 (C) GEMINI CLI — Администратор (эфемерные вызовы)
| Режим | Аргументы | Назначение | Граница |
|-------|-----------|-----------|---------|
| **Console** | `--mode=console` | Инфраструктура, прямой доступ | ❌ Не в пользовательском чате |
| **Secretary** | `--mode=secretary --for=chief\|ceo` | UI-изоляция руководителей, обработка `[TASK]/[INFO]` | ❌ Не лезет в чат сотрудников |
| **Auditor** | `--mode=auditor --scope=decisions\|models` | Аудит по событию/расписанию, рекомендации | ❌ Не фон, не демон — вызов по триггеру |

> ⚠️ **ВСЕ ВЫЗОВЫ АДМИНА — ЭФЕМЕРНЫЕ**  
> Каждый вызов: запускает новый процесс → получает ответ → завершается.  
> Нет долгоживущих процессов. Нет сокетов. Нет состояния между вызовами.
...
┌────────▼────────┐
│ 3. LLM Generation│
│    • Вызов модели или Admin CLI (эфемерный subprocess)
...
├── Gemini/                    
│   ├── gemini_console.txt       # ⚠️ Промпты для Админа (режимы вызова)
│   ├── gemini_secretary_*.txt   # --mode=secretary --for=chief|ceo
│   └── gemini_auditor_*.txt     # --mode=auditor --scope=decisions|models
...
8️⃣ GEMINI CLI — АДМИНИСТРАТОР (ЭФЕМЕРНЫЕ ВЫЗОВЫ)

> ⚠️ **КЛЮЧЕВОЕ ПРАВИЛО:** Все вызовы Админа — эфемерные.  
> Каждый вызов: запускает новый процесс → получает ответ → завершается.  
> Нет долгоживущих процессов. Нет сокетов. Нет состояния между вызовами.

8.1 Архитектура: один бинарник, три режима
Gemini CLI вызывается как эфемерный subprocess с аргументами:

8.2 Вызов Админа из app2.py (эфемерный subprocess)
```python
# В app.py, при необходимости вызова Админа:
from core.gemini_cli_executor import execute

response = await execute(
    mode="secretary",      # или "console", "auditor"
    for_role="chief",      # только для Secretary: "chief" или "ceo"
    scope="decisions",     # только для Auditor: "decisions" или "models"
    message=user_message,  # сообщение с контекстом
...
• Если получен флаг --finalize → пропустить задачу, но залогировать предупреждение
⚠️ Важно: Secretary не хранит состояние между вызовами. Контекст передаётся явно в --message.
...
│
▼
Secretary (Clarity Filter) `--mode=secretary --for=ceo`
│
▼
Auditor `--mode=auditor --scope=decisions` (эфемерный вызов)
│
├── FAST_CHECK режим
│   • 1 модель из пула
...
│
└── Вызов по событию, не фон
• Запускается только при явном запросе или по расписанию
...
│  • Кнопка"console>_" → вызов Admin `--mode=console` │
└────────────┬────────────────────────┘
│
┌────────────▼────────────────────────┐
...
│  • POST /admin         # Вызов Admin (эфемерный)│
│  • GET  /events        # События (опционально)│
└────────────┬────────────────────────┘
│
┌────────────▼────────────────────────┐
│  Admin CLI (эфемерные вызовы)       │
│  Функции:                           │
│  • `--mode=console` — инфраструктура│
│  • `--mode=secretary --for=chief\|ceo` — UI-изоляция│
│  • `--mode=auditor --scope=decisions\|models` — аудит│
│  Безопасность:                      │
│  • Эфемерный процесс: запуск → ответ → завершение│
│  • Нет состояния между вызовами    │
│  • Контекст передаётся явно в `--message`│
└─────────────────────────────────────┘
...
4. Эфемерность вызовов Admin
Каждый вызов Admin (`--mode=...`) — новый процесс
Запрещено: хранить состояние между вызовами
Запрещено: передавать данные между режимами внутри процесса
Контекст передаётся явно: `--message`, `--prompt`, `--project-path`
...
Gemini CLI (Admin) недоступен	Вернуть fallback-ответ, залогировать ошибку, не прерывать чат	Проверить путь к бинарнику, аргументы, права
...
2. CEO (через Admin --mode=secretary --for=ceo) уточняет формулировку
3. При необходимости: Admin --mode=auditor --scope=decisions запускает консультацию
При необходимости: Strategic Orchestrator запускает Red Team консультацию
...
CEO:                   {project_path}/prompts/core/ceo.txt
Chief:                 {project_path}/prompts/core/chief.txt
Отделы:                {project_path}/prompts/department_{A-D}/{role}.txt
Admin (3 режима):      {project_path}/prompts/Gemini/gemini_console.txt, gemini_secretary_*.txt, gemini_auditor_*.txt
Консультанты:          {project_path}/prompts/department_E/consultant_strategic.txt, {project_path}/prompts/department_F/consultant_medical.txt

# Сервер
FastAPI app:           server/app2.py
...
Сервер не стартует / Admin CLI падает	core/app_state.py, core/gemini_cli_executor.py	Проверить project.ini, путь к CLI, аргументы --mode/--prompt
...
# Вызов Админа (режим audit)
POST /admin?mode=auditor&scope=decisions
...
Приложение.
ГДЕ ЖИВЁТ «ПАМЯТЬ» (АРХИТЕКТУРНАЯ КАРТА)
Уровень	Что хранит	Как передаётся Админу
app.py (оркестрация)	Сессии, история диалога, текущий task_id	Форматирует --message с контекстом
ChatStorage	Файлы чатов: chats/doctor/session_*.json	Загружает историю, передаёт релевантный фрагмент
PromptRegistry	Системные промпты ролей	Передаёт --prompt или путь к файлу
ProjectContext	Пути к проекту, настройки	Передаёт --project-path
Gemini CLI (Админ)	НИЧЕГО (эфемерный процесс)	Получает всё явно → отвечает → умир
