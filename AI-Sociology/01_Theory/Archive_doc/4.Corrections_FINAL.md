...
**Статус:** канонический пакет правок. Заменяет `Corrections.md` и накопительный файл `REVISIONS` целиком. Оба поглощены; прилагать их к передаче не нужно.

**Что это.** Перечень изменений к каноническому тексту (Blocks 1–5). Не рукопись. Каждая запись указывает, что именно замещается, вставляется или удаляется.

**Кому.** Manuscript Composer — как обязательная спецификация. Witness of the previous manuscript — как справочный документ, определяющий, какие формулировки сняты.

**Содержание**

- Часть I. Замены целых разделов (§4, §8)
- Часть II. Замены протоколов (§11.4: E6, E8)
- Часть III. Точечные правки
- Часть IV. Механические замены при сборке
- Часть V. Снятые утверждения — не возвращать
- Часть VI. Открытые вопросы к Автору

---

# ЧАСТЬ I. ЗАМЕНЫ ЦЕЛЫХ РАЗДЕЛОВ

## I-1. Section 4 — полная замена

**Заменяет:** §4.1–§4.6 в BLOCK 2, от заголовка «# 4. Conceptual Framework» до строки перед «# 5. The Role-Reconfiguration Experiment».

**Структурные изменения:** §4.2 переписан (введён *menom*, «DNA» удалён из нормативного словаря); §4.5 и §4.6 слиты в §4.5; прежний §4.4 сохраняет номер; ссылка на Кelly перенесена в §8.11.
...
## I-2. Section 8 — три замены и одна вставка

**§8.1–§8.8 остаются без изменений**, кроме §8.7, куда добавляется абзац (I-2d).

### I-2a. §8.9 — полная замена
...
### I-2c. §8.11 — новая вставка

**Вставляется после §8.10, перед заголовком «# 9. Design Principles».**
...
### I-2d. §8.7 — добавление в конец раздела

**Добавляется после последнего абзаца §8.7 («The practical consequence is that the architecture does not require a hierarchy of authority…»).**
...
# ЧАСТЬ II. ЗАМЕНЫ ПРОТОКОЛОВ

**Заменяет в BLOCK 5, §11.4:** подразделы «### E6 — Attribution, in this regime» и «### E8 — Two axes of heterogeneity» целиком, плюс строку приоритета. E1–E5, E7 без изменений.
...
## III-1. Формулировка паттерна — три места
...
**§5.9**, последний абзац — заменить целиком:
...
## III-2. Счёт для промптов A и B

**§5.4, C2.** Заменить пункт «Quantity of institutional fiction» целиком:
...
**§3.5**, перечень словаря. Было: *«role, identity, resistance, colleague, institution, behavioural prior»* → *«role, identity, resistance, colleague, institution, menom»*

**§6.4**, третий абзац — заменить целиком:
...
**§12.1**, «Anthropomorphic vocabulary». Было: *«…institution and behavioural DNA are used functionally…»* → *«…institution and menom are used functionally…»*

**При сборке:** проверить и устранить все прочие вхождения «DNA» вне §4.2.2.

## III-4. Маркеры ⚠ вне §2

**§2.0**, добавить последним абзацем:
...
> …and ⚠ agents are reported to accommodate one another's stated positions in ways reduced by anonymizing the source (§2.5).

Маркеры в §8.9 и §11.4 (E6) уже проставлены в частях I и II.

## III-5. Номера экспериментов и приложение
...
**§5.2**, Step 2, последняя фраза. Было: *«Exact replacement texts in Appendix A, §7–8.»* → *«Exact replacement texts are reproduced in Appendix A.»*

## III-6. Хост-проект

**§2.11**, последний абзац — заменить целиком:
...
**§12.1**, вставить после пункта «Single project»:
...
## III-8. §3.8 — новая вставка

**Вставляется после §3.7 (Dates).**
...
1. **`§4.6` → `§4.5`** — семь вхождений: §1.4, §2.9, §5.5 (дважды), §5.8, §8.9, §10.3. Конструкты *represented social position* и *represented social source* после правки I-1 находятся в §4.5.4.

2. **`behavioural DNA`, `behavioral DNA`, `model DNA`** — устранить везде, кроме §4.2.2.

3. **`behavioural prior`** — сохраняется только там, где обозначает наблюдаемое поведение семейства; где обозначает гипотетический механизм, заменяется на *menom*. Проверить §6.4, §12.1.

4. **Титульный блок** — принят в редакции:
...
5. **Пометка в §1** — вставить в конце §1.1 или §1.2, по усмотрению Сценариста:
...
Тринадцать позиций. Ни одна не восстанавливается ни в какой формулировке.

| Снято | Причина | Где закрыто |
|---|---|---|
| Kelly как обоснование числа три | метод элицитации осей, не утверждение о составе группы | §8.11.4 |
| «AI Ethology» как новая дисциплина | занято программой machine behaviour | §2.7 |
| «Grok immediately accepted the new specialization» | описывает Шаг 2, противоречит протоколу | §5.3 |
| «Одно семейство принимает роли, другое сопротивляется» | оба сохранили траекторию | §5.3, §6.4 |
| Триады «возникали естественно» | все изменения вносил один координатор | §8.8 |
| Пары накапливают слепые зоны / три — минимум | противоречит Уставу, датированному раньше | §8.1–8.2 |
| «Сохранение метода → переход удался» как вывод | конфаунд C3 | §5.4, §6.2 |
| Коллективная память как находка | транзактивная память, распределённое познание | §2.4 |
| Bounded rationality Саймона как источник | ограничены ресурсы агента, не информация | §2.6 |
| «Более сильные модели работают хуже» как принцип | контрпример внутри статьи | §7.4 |
| «Behavioural DNA» как нормативный термин | метафора репликации противоречит механизму развития | §4.2.2 |
| Заявка на первое использование термина «AI Sociology» | термин существует с 1994 | §2.8 |
| «Each new model version is trained anew» | не установлено и не универсально | §4.2.3 |

Плюс: логи с частотами и подсчётом снов из материалов Морфохаба — художественный текст внутри постановки, не цитируется ни в каком виде.

---

# ЧАСТЬ VI. ОТКРЫТЫЕ ВОПРОСЫ К АВТОРУ

Четыре. Ни один не блокирует композицию; все требуют решения до публикации.

**VI-1. §3.6 — фактический состав участников.** Раздел описывает композицию как выполненную отдельным участником с последующим независимым ревью. Приведение в соответствие — после того, как состав будет зафиксирован фактически.

**VI-2. §5.5 — деталь об уступке.** Пункт 3 фиксирует, что уступка распространилась за пределы редакционного спора на научные утверждения о принимающей теории. Это сильнейшая часть наблюдения и одновременно запись о том, что модель подтвердила непроверенные утверждения о теории Автора. Решение о сохранении — за Автором.

**VI-3. §5.6 — материалы near-control.** Требуется транскрипт плюс подтверждение, что возобновление не сопровождалось системным промптом или ролевой инструкцией.

**VI-4. §5.7 — число чистых прогонов.** Источник допускает чтение как об одном прогоне, так и о двух. Разница между n = 1 и n = 2 против одного положительного случая.

**Плюс:** все ссылки, помеченные ⚠ в §2, требуют открытия страниц перед публикацией. Подтверждены четыре: Rahwan, Swanson, Du, Zhang. От верификации Choi зависит формулировка E6.
